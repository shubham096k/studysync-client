...
import AssignmentIcon from "@mui/icons-material/Assignment"; // task icon
...
export default function AddTask() {
  ...
  const [sessionData, setSessionData] = useState(null); // State for session data
  const [groupData, setGroupData] = useState(null); // state for group details

  useEffect(() => {
    if (sessionId) {
      fetchTasks();
      fetchSessionData(); // Fetch session data
    }
  }, [sessionId]);
  ...
  const fetchSessionData = async () => {
    try {
      ...
      // Fetch group details after getting session data
      if (res.data.group) {
        fetchGroupData(res.data.group);
      }
    } catch (error) {
  ...
  return (
    <Box p={3} maxWidth={800} mx="auto">
      ...
      {/* Session Name and Description Section */}
      {sessionData && (
        <Box sx={{ mb: 4 }}>
